...
if __name__ == '__main__':
    ...
    pr_aucs = []
    for train_index, test_index in skf.split(exposure_df, feature_df["HR Status"]):
        X_train, X_test = exposure_df.iloc[train_index], exposure_df.iloc[test_index]
        y_train, y_test = feature_df[gene].iloc[train_index], feature_df[gene].iloc[test_index]
